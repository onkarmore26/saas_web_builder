...
import Link from "next/link";
import { useRouter } from "next/navigation";
import React, { FocusEventHandler, useEffect, useCallback } from "react";
import { toast } from "sonner";
...
const FunnelEditorNavigation = ({
      ...
      async (event) => {
        ...
        if (newTitle) {
          try {
            ...
            toast.error("Error saving funnel page title");
          }
        } else {
  ...
  }, [subaccountId, funnelPageDetails, funnelId, state.editor.elements]);

  return (
    <TooltipProvider>
      <nav
        className={clsx(
          "border-b-[1px] flex items-center justify-between p-6 gap-2",
          { "!h-0 !p-0 !overflow-hidden": state.editor.previewMode }
        )}
      >
        <aside className="flex items-center gap-4 max-w-[260px] w-[300px]">
        ...
        <aside>
          ...
          >
            <TabsList className="grid w-full grid-cols-3 bg-transparent h-fit">
              <TabsTrigger value="Desktop" className="w-10 h-10 p-0">
                <Laptop />
              </TabsTrigger>
              <TabsTrigger value="Tablet" className="w-10 h-10 p-0">
                <Tablet />
              </TabsTrigger>
              <TabsTrigger value="Mobile" className="w-10 h-10 p-0">
                <Smartphone />
              </TabsTrigger>
            </TabsList>
          </Tabs>
